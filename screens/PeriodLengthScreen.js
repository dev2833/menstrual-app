...
export default function PeriodLengthScreen({navigation, route}) {
  ...
  React.useEffect(() => { 
    setPeriodLength(users[currentUser].periodLength)
  }, []);


  function updatePeriod(type){
      if(type=="minus" && periodLength!=1){
        const next = parseInt(periodLength)-1
  ...
  async function updatePeriodDetails(){
    users[currentUser].periodLength = periodLength;
    console.log(users)
    dispatch({ type: 'SET_USER', data: users });
    await AsyncStorage.setItem("users", JSON.stringify(users));
    navigation.navigate("setting", {period: periodLength, cycle: users[currentUser].cycleLength})
  }
    
  return (
    <View style={{flex:1, backgroundColor: '#fff'}}>
        <View style={{flex:1, margin: 10, }}>
            <Text style={{fontSize: 16, color: '#605e5e', marginBottom: 10, marginLeft: 20}}>{config.Labels[language].set_period_l}</Text>
            ...
            <Text style={{fontSize: 14, color: '#605e5e', marginBottom: 5, marginTop: 20, marginLeft: 10}}>
                {config.Labels[language].dash_bleed_bet}
            </Text>
